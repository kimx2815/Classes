/* CSci 5105 Bulltein Board System
 * Francis Liu, Taehun Kim
 * client.cpp
 */
#include <stdio.h>
#include <stdlib.h>
  ...
  while(1) 
    {
      ...
      printf("[1]POST [2]READ [3]Choose [4]Reply\n");  
      printf("[5]Test-post [6]Test-post&reply\n>>");
      ...
      if(select == '1')
	{
	  ...
	  cout << " POST "<<endl;
	  cout << "Article : ";
	  cin.getline(article,1000);//cin.ignore();
	  int n;
	...
	{
	  printf("TEST article generate\n");

	  int i ;
	  char article_T[1000] = "TEST1article";
	  unsigned short article_len = strlen(article_T);

	  char buf_5[BUFSIZE];
	  char buf_r[BUFSIZE];
	  buf_5[0] = POST_REQ;
	  buf_5[1] = article_len & 0xff;
	  buf_5[2] = (article_len>>8) & 0xff;
	  strcpy(&buf_5[3], article_T);
	  cout << "TEST : POST_REQ  " << endl;
	  ...
	  for(i=1; i< 100; i++) 
		...
		fprintf(stderr,"[");
	      sleep(1);
	      send(sockfd, buf_5, article_len+3, 0);
	      
	      bzero(buf_r, BUFSIZE);
	      
	      recv(sockfd, buf_r, BUFSIZE, 0);	  
	      fprintf(stderr,".");
	    }
	  printf("]\nTest_POST_Done\n");
	}
	...
	{
	  printf("TEST article generate v0.2\n");

	  int i ;
	  char article_T[1000] = "TEST2article";
	  unsigned short article_len = strlen(article_T);
	  int re_article;
	  char buf_4[BUFSIZE];
	  char buf_5[BUFSIZE];
	  ...
	  for(i=1; i< 100; i++) 
		...
		{
		 sleep(1);
		  send(sockfd, buf_4, article_len+MIN_REPLY_REQ_LEN, 0);	 bzero(buf_r, BUFSIZE);
		  recv(sockfd, buf_r, BUFSIZE, 0);
		  fprintf(stderr,"*");
		  sleep(1);
		}
